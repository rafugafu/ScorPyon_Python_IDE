...
try:
	...
	image = tk.PhotoImage(file = '/usr/share/ScorPyon/Icon.png')
	...
	def ssc(event = None):
		...
		if answer == 'yes':
			try:
				ld('/usr/share/ScorPyon/main.py')
				clt('/usr/share/ScorPyon/main.py')
	...
	h.add_command(label = 'Author', command = rafey)
	ha()
	if len(sys.argv) > 1:
		ld(sys.argv[1])
	root.mainloop()
