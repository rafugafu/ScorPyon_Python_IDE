...
# import getpass (if local installation)
import os
# os.chdir(f'/home/{getpass.getuser()}/.local/share/ScorPyon/') (if local installation)
...
try:
		...
		if error != None:
			...
			if not os.popen(interpreter + '  temporary.py').read() == '':
				if tfgoe == True:
					...
					shellout.config(state=tk.DISABLED)
				hist.insert('end', os.popen(interpreter + ' ' + 'temporary.py').read())
	...
	def save_run(event = None):
		...
		if error != None:
			if tfgoe == True:
				...
				shellout.tag_config('error', foreground = 'red')
				tabs.select(shell_tab)
				shellout.config(state = 'disabled')
			hist.insert('end', error)
		else:
			if not os.popen(interpreter + ' ' + title_global).read() == '':
				if tfgoe == True:
					...
					tabs.select(shell_tab)
					shellout.config(state = tk.DISABLED)
				hist.insert('end', os.popen(interpreter + ' ' + title_global).read())
	def lld():
	...
	def shchng():
		root = tk.Tk()
		root.title('Changes')
		list = ['1. History tab', '2. Bug where you could sometimes edit shell output fixed', '3. New menu "Help"', '4. Version shown in "Options -> Version"', '5. Bug where source code could not be opened through options menu fixed']
		ch = tk.Listbox(root)
		ch.pack(fill = 'both')
		for item in list:
			ch.insert('end', item)
	...
	h.add_command(label = 'Changes', command = shchng)
	ha()
	if len(sys.argv) > 1:
		ld(sys.argv[1])
	root.mainloop()
