...
# user = getpass.getuser() (if local installation)
...
try:
	...
	def set_intp(inpt):
		global interpreter
		list = ['python3', 'python3.6', 'python3.7', 'python3.8', 'python3.9', 'python3.10']
	...
	def prf(event = None):
		...
		ttk.Label(it, text = 'If it is not a valid input, the interpreter will not change. Make sure the interpreter is included in your system PATH.').grid(column = 1, row = 0)
	def goe():
		global tfgoe
		global shellout
		global out
	...
	h.add_command(label = 'Version', command = shv)
	ha()
	if len(sys.argv) > 1:
		ld(sys.argv[1])
	root.mainloop()
