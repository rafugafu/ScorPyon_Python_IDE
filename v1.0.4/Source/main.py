...
from runner import ragoae_other
...
try:
	root = tk.Tk()
	root.title(f'ScorPyon {v} - Untitled.py')
	...
	def execute_other():
		...
		runthingy_ = ragoae_other(shellin.get(1.0, 'end'))
	...
	def ha():
		...
		keywords = kw.kwlist
		other_keywords = ['file', 'open', 'map', 'int', 'str', 'print', 'range', 'set', 'input', 'list', 'len', 'self', 'type', 'exec', 'sum', 'iter', 'dir', 'compile', 'eval', 'format', 'locals', 'cls', 'exit', 'dict', 'repr', 'hasattr', 'setattr', 'super', 'isinstance', 'object', 'tuple', 'float']
		if hk == True:
	...
	def highlight_brackets_and_inside():
		...
		while True:
			...
			n = nn
		shellin.tag_config('bf', foreground = fbc, background = bbc)
	def save_run_other(event = None):
		sssv()
		global tfgoe
		global title
		global hist
		runthingy_ = ragoae_other(code.get(1.0, 'end'))
		if type(runthingy_[1]) != str:
			runthingy = [False]
			str_ = ''
			for item in runthingy_[1]:
	...
	def clt(nt):
		...
		if not nt == '':
			try:
				root.title(f'ScorPyon {v}' + ' - ' + os.path.basename(nt))
			except:
				pass
		else:
			root.title(f'ScorPyon {v} - Untitled.py')
	...
	def return_val(event):
		...
		global shell_in
		global tabs
		shellin.delete(1.0, 'end')
		shellin.insert('end', hist.get(hist.curselection()))
	...
	def kb(event = None):
		...
		kbw = tk.Frame(kbw_)
		kbw.pack()
		tk.Label(kbw, text = '1. It does not open filenames with a space from the terminal (linux)').grid(sticky = 'w', column = 0, row = 0, padx = 10, pady = 10)
		tk.Label(kbw, text = '2. Does not work as an application on windows').grid(sticky = 'w', column = 0, row = 1, padx = 10, pady = 10)
	...
	def changes(event = None):
		...
		cw = tk.Frame(cw_)
		cw.pack()
		tk.Label(cw, text = '1. A version of अजगर is included. If you are using it, it will produce a translated file in the same directory, which can be used with normal python.').grid(sticky = 'w', column = 0, row = 1, padx = 10, pady = 10)
		tk.Button(cw_, text = 'Close', command = cw_.destroy).pack(fill = 'x', side = 'bottom', padx = 10, pady = 10)
		cw_.mainloop()
	def term(event = None):
		termwin = tk.Tk()
		termwin.title('Terminal (Beta)')
		tk.Label(termwin, text = user + ':' + os.path.basename(os.getcwd()) + '/$').grid(column = 0, row = 0)
		cmd = tk.Entry(termwin)
		cmd.grid(column = 1, row = 0)
		out = tk.Label(termwin)
		out.grid(column = 0, row = 1)
		cmd.bind('<Return>', lambda event: [os.system(cmd.get() + ' > /usr/share/ScorPyon/term.txt'), cmd.delete(0, 'end'), out.config(text = open('/usr/share/ScorPyon/term.txt').read()), os.remove('/usr/share/ScorPyon/term.txt')])
		cmd.focus()
		termwin.mainloop()
	tabs = ttk.Notebook(root)
	code_tab = ttk.Frame(tabs)
	tabs.add(code_tab, text = 'Code')
	code_tab_ = tk.Frame(code_tab)
	code_tab_.grid(column = 0, row = 0)
	shell_tab = ttk.Frame(tabs)
	tabs.add(shell_tab, text = 'Shell')
	hist_tab = ttk.Frame(tabs)
	tabs.add(hist_tab, text = 'History')
	tabs.pack(fill = 'both')
	scrolly = tk.Scrollbar(code_tab_)
	scrolly.pack(side = 'right', fill = 'y')
	code = tk.Text(code_tab_, wrap = tk.WORD, fg = f, bg = b, insertbackground = 'white', yscrollcommand = scrolly.set, height = 50)
	code.pack(fill = 'both')
	scrolly.config(command = code.yview)
	code.insert('end', '# \'ScorPyon\' Python IDE')
	code.bind('<KeyRelease>', kp)
	code.bind('<Button-3>', right_clicked_main_code)
	shelltop = tk.Frame(shell_tab)
	shelltop.grid(column = 0, row = 0)
	shellbottom = tk.Frame(shell_tab)
	shellbottom.grid(column = 0, row = 1)
	tk.Label(shelltop, text = 'In:', font = ('TkDefaultFont', 20)).pack(side = 'left', padx = 10, pady = 10)
	shellscrollytop = tk.Scrollbar(shelltop)
	shellscrollytop.pack(side = 'right', fill = 'y')
	shellin = tk.Text(shelltop, wrap = tk.WORD, fg = f, bg = b, insertbackground = 'white', yscrollcommand = shellscrollytop.set)
	shellin.pack(anchor = 'e')
	shellscrollytop.config(command = shellin.yview)
	shellin.bind('<KeyRelease>', kp)
	tk.Label(shellbottom, text = 'Ot:', font = ('TkDefaultFont', 20)).pack(side = 'left', padx = 10, pady = 10)
	shellscrollybottom = tk.Scrollbar(shellbottom)
	shellscrollybottom.pack(side = 'right', fill = 'y')
	shellout = tk.Text(shellbottom, wrap = tk.WORD, fg = f, bg = b, insertbackground = 'white', yscrollcommand = shellscrollybottom.set, state = tk.DISABLED)
	shellout.pack(anchor = 'e')
	shellscrollybottom.config(command = shellout.yview)
	...
	tk.Button(shell_tab, text = 'Execute', font = ('Sans', 20), command = execute).grid(column = 0, row = 3)
	tk.Button(shell_tab, text = 'Execute - अजगर', font = ('Sans', 20), command = execute_other).grid(column = 0, row = 4)
	ttk.Label(hist_tab, text = 'History:').pack()
	hist = tk.Listbox(hist_tab, height = 50)
	hist.pack(fill = 'both')
	...
	fm.add_command(label = 'Save & Run - अजगर', command = save_run_other)
	fm.add_separator()
	...
	o.add_separator()
	o.add_command(label = 'Terminal (Beta)', command = term)
	root.bind('<Control-p>', prf)
	o.add_separator()
	o.add_command(label = 'Source code  ->  Ctrl + O', command = ssc)
	root.bind('<Control-o>', ssc)
	h = tk.Menu(m)
	m.add_cascade(label = 'Help', menu = h)
	h.add_command(label = 'About ScorPyon', command = about)
	h.add_command(label = 'Known problems', command = kb)
